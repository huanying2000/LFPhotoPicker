...
typedef enum : NSUInteger {
    LFPhotoManagerSelectedTypePhoto = 0,//只选择图片
    LFPhotoManagerSelectedTypeVideo, // 只选择视频
    LFPhotoManagerSelectedTypePhotoAndVideo // 图片和视频一起
} LFPhotoManagerSelectedType;

@interface LFPhotoManager : NSObject

//是否开启3Dtouch 预览功能 默认打开
@property (nonatomic,assign) BOOL open3DTouchPreview;

//显示全屏相机 默认NO
@property (nonatomic,assign) BOOL showFullScreenCamera;
...
/*
当选择类型为 LFPhotoManagerSelectedTypePhotoAndVideo 时 此属性为YES时 选择的视频会跟图片分开排  反之  视频和图片混合在一起排
*/
...
//------//
@property (assign, nonatomic) LFPhotoManagerSelectedType type;
...
/**
 ...
 @param model 模型
 */
- (void)deleteSpecifiedModel:(LFPhotoModel *)model;
